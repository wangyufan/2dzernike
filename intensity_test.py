...
def calc_intensity_by_mask_0121(mask, mat, i):
	...
	if (len(mat_flat) == len(mask_flat)):
		...
		for i in range(len(mask_flat)):
			...
			if (int(mask_flat[i]) == 1):
				...
				signal += pixel
				signal_count +=1
		if ((int(signal/signal_count) - int(background/background_count)) > 0):
		...
		print("intensity_avg= ",int(np.divide(signal, signal_count) - np.divide(background, background_count)))
		print("signal=    ",int(signal))
		print("background=",signal_count*np.divide(background, background_count))
		print("intensity= ",signal - signal_count*np.divide(background, background_count))
	return intensity
...
def findTemplate(perSize, process_n, node_n, xy_mat, hash_type, template_path, base_path):
	...
	for index in range(start_line, end_line):
		...
		i_arr = [x[4] for x in xy_mat[index]]
		...
		f_cxi.close()
		for (x, y, i) in zip(x_arr, y_arr, i_arr):
			...
			for j in range(len(template_data)):
				...
				if hash_cmp > hash_max:
					hash_max = hash_cmp
					find_template = j+1
			# if(int(y)==854 and int(x)==309):
			# if(int(y)==1196 and int(x)==354) or (int(y)==854 and int(x)==309) or (int(y)==904 and int(x)==320) or (int(y)==410 and int(x)==290) or (int(y)==515 and int(x)==226) or (int(y)==685 and int(x)==250):
			...
			mask = template_mask[find_template-1]
			intensity = calc_intensity_by_mask_0121(mask, image_mat, i)
...
def get_xy(filepath):
    ...
    for line in lines:
    	...
    	if (len(linesplit) == 10 and flag_hkl):
            ...
            Y_hkl = linesplit[7]
            peak+=1
            one_frame_xy_mat.append((cxi_num, frame_num, int(float(X_hkl) + 0.5), int(float(Y_hkl) + 0.5), intensity_hkl))
...
if __name__ == "__main__":
	...
	perc = os.path.basename(template_mask_path).split('_')[1]
	...
	for process_n in range((node_n-1)*process_num, node_n*process_num):
		elem = pool.apply_async(findTemplate, (perSize, process_n, node_n, frame_mat, 1, template_path, base_path))
		arr.append(elem)
	pool.close()
	pool.join()
	...
	# intensity_path = '/Users/wyf/Documents/100f_result/'+str(perc)+ '_intensity.npy'
	intensity_path = '/Users/wyf/Documents/100f_result/3_intensity.npy'
